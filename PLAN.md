...
- Extensions are iframed panels registered against a project via a **manifest URL**. Confirmed schema (Trimble's published [`ExtensionSetting` reference](https://components.connect.trimble.com/trimble-connect-project-workspace-api/docs/interfaces/extensionsetting.html)): `title` and `url` are **required**; `description`, `icon`, `enabled`, `configCommand` are optional. There's no `name`, `extensionType`, `vendor`, `id`, or `version` field — a manifest missing `title` is rejected outright ("not a valid extension"), which is what happened during this build. A project admin adds the manifest URL under **Project Settings → Apps & Capabilities → Add Custom**. The manifest URL must be served over HTTPS and be CORS-enabled, since Connect fetches it directly.
